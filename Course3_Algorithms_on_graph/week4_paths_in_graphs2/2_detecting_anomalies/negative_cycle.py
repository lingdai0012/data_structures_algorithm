...
def bellman_ford(dist, prev, edges):
    ...
    return dist, prev, updated_index
...
def negative_cycle(adj, cost):
    ...
    dist = [0 if _ == s else math.inf for _ in range(len(adj))]
    prev = [None for _ in range(len(adj))]
    for _ in range(len(adj) - 1):
        dist, prev, _ = bellman_ford(dist, prev, edges)

    dist_last, prev_last, last_updated_index = bellman_ford(dist, prev, edges)
    if len(last_updated_index) != 0:
        return 1
    return 0
