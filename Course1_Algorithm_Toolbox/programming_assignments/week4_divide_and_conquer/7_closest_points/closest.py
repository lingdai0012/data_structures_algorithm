...
def filter_points_by_midline(x, y, midline, minimum_splitted_distance):
    ...
    filtered_index = list(filter(lambda ii: abs(x[ii]-midline)<=minimum_splitted_distance, index))
...
def half_search(x, y):
    ...
    filtered_x, filtered_y = filter_points_by_midline(x, y, midline, min_spiltted_distance)
    ...
    for ii in range(len(sorted_filltered_y)):
        for jj in range(ii+1, ii+7):
            if jj >= len(sorted_filltered_y):
                break
            temp_distance = calculate_distance(sorted_filltered_x[ii], sorted_filltered_y[ii], sorted_filltered_x[jj], sorted_filltered_y[jj]) 
