...
def get_majority_element(a, left, right):
    ...
    majority = compare_majorities(
        left_majority_element, right_majority_element, a, left, mid, right
    )
    return majority


def compare_majorities(
    left_majority_element, right_majority_element, a, left, mid, right
):
    if left_majority_element == -1 and right_majority_element == -1:
        ...
        if (
            sum([num == right_majority_element for num in a[left:right]])
            > len(a[left:right]) / 2
        ):
        ...
        if (
            sum([num == left_majority_element for num in a[left:right]])
            > len(a[left:right]) / 2
        ):
            return left_majority_element
        else:
